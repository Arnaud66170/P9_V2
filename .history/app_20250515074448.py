...
def contains_intense_words(text):
    words = re.findall(r"\w+", text.lower())  # extrait tous les mots alphanumériques en minuscules
...
def predict_emotions(text):
    ...
    if not filtered:
        result = "😐 Aucune émotion détectée"
    else:
        filtered.sort(key=lambda x: x[1], reverse=True)
        result = ", ".join(f"{l} ({s:.2f})" for l, s in filtered)

    top = filtered[0] if filtered else ("neutral", 0.0)
    history.appendleft({"text": text, "emotion": top[0], "score": round(top[1]*100, 2)})

    alert_banner = ""
    if contains_intense_words(text):  # 🔁 déclenchement uniquement basé sur présence d'un mot intense
        suspect_text = text
        alert_banner = "🟥 <strong style='color:red;'>⚠️ Ce tweet contient un mot intense. Consultez l’onglet \"Cas suspects\" pour audit.</strong>"

    html_output = f"<h2 style='text-align:center;'>🧠 Résultat : {result}</h2>"
    if alert_banner:
        html_output += f"<div style='margin-top:10px'>{alert_banner}</div>"
...
def audit_text(text):
    ...
    filtered.sort(key=lambda x: x[1], reverse=True)
    alert = ""
    if contains_intense_words(text) and (not filtered or filtered[0][0] == "neutral"):
        alert = "⚠️ Attention : émotion absente malgré termes forts\n"
    res = alert + ", ".join(f"{l} ({s:.2f})" for l, s in filtered)
    return res or "😐 Aucune émotion détectée", {l: s for l, s in filtered}
