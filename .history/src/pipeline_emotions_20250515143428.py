...
class CustomTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss_fct = BCEWithLogitsLoss(weight=self.class_weights_tensor.to(outputs.logits.device))
...
def run_emotion_pipeline(force_retrain: bool = False, test_mode: bool = False) -> Tuple:
    """
    Entraîne ou améliore un modèle ELECTRA (multi-label, 28 émotions) sur GoEmotions.
    Peut tourner en mode test rapide via `test_mode=True`.
    """
    ...
    # ➤ Réduction du nombre d’epochs en mode test
    ...
    with mlflow.start_run(run_name="goemotions_electra_test" if test_mode else "goemotions_electra_optimized"):
        ...
        # if test_mode:
        #     print("⏩ Mode test activé : entraînement SKIPPÉ")
        # else:
