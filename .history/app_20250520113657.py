...
def generate_emotion_summary():
    if not history:
        return None
    df = pd.DataFrame([h['emotion'] for h in history], columns=["emotion"])
...
def download_emotion_stats():
    return EXPORT_XLSX_PATH if os.path.exists(EXPORT_XLSX_PATH) else None
...
with gr.Blocks() as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("📁 Export / Logs"):
            gen_button = gr.Button("📤 Générer .xlsx résumé émotions")
            download_btn = gr.File(label="Fichier .xlsx", interactive=True)
